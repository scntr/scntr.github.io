...
$(document).ready(function(){
	$('#start-raffle').click(startRaffle);
...
var makeTicketsWithPoints = function(){
	...
	setTimeout(function() {
		...
		$('body').unbind('click');
		$('#start-raffle').prop('disabled', false);
		$('.ticket').unbind('click').click(startRaffle);
	}, 500);
}
...
function startRaffle(event){
	...
	if(inProgress || getChoices().length === 0)
		return;

	$('#start-raffle').prop('disabled', true);
...
var pickName = function(){
	...
	if(choices.length > 1){
		...
		choices.click(function(){
			...
			for(var i = imported.length - 1; i >= 0; i--) {
   				if(imported[i].name === choices[0].innerText) {
       				imported.splice(i, 1);
       				 $("#winnerContainer").append("<div class=\"winners\">"+choices[0].innerText+"</div>");
    			}
			}
